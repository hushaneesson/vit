<div>
    <div class="overflow-hidden bg-white border border-gray-200 shadow-sm rounded-xl">
        ...
        {{-- Scrollable wrapper: everything below scrolls together horizontally --}}
                        ...
                        ">
                            ...
                            <div class="space-y-1">
                                @foreach (collect($day['slots'])->take(3) as $slot)
                                    <button type="button"
                                        wire:click="selectSlot('{{ $day['date']->format('Y-m-d') }}', '{{ $slot }}')"
                                        class="
                                        w-full
                                        px-1.5
                                        py-1
                                ...
                                @if (count($day['slots']) > 3)
                                    <button type="button" wire:click="viewSlots('{{ $day['date']->format('Y-m-d') }}')"
                                        class="
                                        w-full
                                        px-1.5
                                        py-1
                                ...
                                @if (empty($day['slots']) && $day['date']->month === $month)
                                    <span class="block mt-2 text-[11px] text-gray-400 whitespace-nowrap">
                                        No availability
                                    </span>
                                @endif
                            </div>
        ...
        <div class="relative w-full max-w-md p-6 bg-white shadow-xl rounded-xl" x-show="$wire.showModal" x-transition>
            ...
            <div class="space-y-2">
                @foreach ($selectedSlots ?? [] as $slot)
                    <button type="button" wire:click="selectSlot('{{ $selectedDate }}', '{{ $slot }}')"
    ...
    <div x-data x-show="$wire.showConfirmModal" x-cloak x-on:keydown.escape.window="$wire.closeConfirmModal()"
        class="fixed inset-0 z-50 flex items-center justify-center px-4">
        <div class="fixed inset-0 transition-opacity bg-gray-900/50" x-on:click="$wire.closeConfirmModal()"
            x-show="$wire.showConfirmModal" x-transition.opacity></div>

        <div class="relative w-full max-w-sm p-6 bg-white shadow-xl rounded-xl" x-show="$wire.showConfirmModal"
            x-transition>
            <h3 class="mb-1 text-sm font-semibold text-gray-800">Confirm Appointment</h3>

            <p class="py-2 mb-4 text-sm text-gray-500">
                @if ($pendingDate)
                    {{ \Carbon\Carbon::parse($pendingDate)->format('l, F j, Y') }}
                @endif
                @if ($pendingSlotLabel)
                    at {{ $pendingSlotLabel }}
                @endif
            ...
            <div class="flex gap-2">
                ...
                <button type="button" wire:click="confirmAppointment" wire:loading.attr="disabled"
                    ...
                    class="flex-1 px-4 py-2 text-sm font-semibold text-white rounded-lg bg-sky-600 hover:bg-sky-700 disabled:opacity-50">
                    <span wire:loading.remove wire:target="confirmAppointment">Confirm</span>
                    <span wire:loading wire:target="confirmAppointment">Saving...</span>
                </button>
            </div>
        </div>
    </div>
